...
from utiles.fechas import last_day_n_months_ago_by_year, last_day_n_months_ago
import numpy as np
...
def create_returns_pivot_table(df: pl.DataFrame, pivot_values: str = "RENTABILIDAD_ACUMULADA") -> pl.DataFrame:
    """
    ...
    df_with_id = df.with_columns(
        pl.concat_str([pl.col("RUN_FM"), pl.col("SERIE")], separator=" - ").alias("FONDO_SERIE")
    )
...
def calculate_relative_returns(pivot_df: pl.DataFrame) -> pl.DataFrame:
    ...
    for col in numeric_cols:
        ...
        result_exprs.append(
            (last_value / pl.col(col) -1 ).alias(f"{col}")
        )
...
if __name__ == "__main__":
    ...
    print(stats_df)
    stats_df.write_csv("estadisticas_por_fila.csv")
